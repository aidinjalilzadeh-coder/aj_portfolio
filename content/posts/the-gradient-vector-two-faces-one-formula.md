...
The key to mastering the gradient is learning to switch between these two perspectives: the contour map and the 3D surface. A point on the contour map corresponds to a point on the surface vertically aligned with it — either above or below the $xy$-plane, depending on the sign of $z = f(x,y)$. For instance, the point $(0,4)$ on the map corresponds to $(0,4,2.22)$ on the surface. The explicit gradient vector, which lives in the $xy$-plane, guides your steps on the map. But when you lift that same point onto the surface, the geometry changes.
...
<figure style="display: flex; flex-wrap: wrap; justify-content: center; gap: 10px;">
  ...
  >
  <figcaption style="width: 100%; text-align: center; margin-top: 8px;">
    <strong>Figure 2:</strong> Different views of gradient for a fixed point. The thick orange curve on the lower plot is the path of steepest change. Takes you uphill if you follow along the blue arrow.
  </figcaption>
</figure>
