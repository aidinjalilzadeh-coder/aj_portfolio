...
$$
where $\mathcal{E}$ is the error (See Figure (1)). The equality $\int\_a^b f(t) , \text{d}t = \int\_a^b f(a), \text{d}t$ holds only if $f(t)$ is a constant function over $\[a,b]$. A constant function can be thought of a polynomial of **degree zero**. Hence the reason this numerical quadrature is of degree zero.
