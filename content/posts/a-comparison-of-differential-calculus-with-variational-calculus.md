...
We know that the **shortest path** connecting two distinct points in 2-D or 3-D space is a **straight line**. This rather trivial fact is indeed a variational problem which goes back to Euclid and he proves it early on in Book I. He does so by first proving the triangle inequality showing in any triangle each side is shorter than the sum of the other two. It was a clever proof for its time but it was limited polygonal paths only since he couldn't calculate the length of a curved path!
