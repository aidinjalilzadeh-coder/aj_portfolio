...
Now, let's do calculus: suppose we have a single variable function $y=f(x)$ and $x=a$ is an extremum point of $f$; therefore we must have $f'(a)=0$. But this is not sufficient for an extreme point if $f$ doesn't change sign at $a$. So we must check the sign of $f''(a)$ 
